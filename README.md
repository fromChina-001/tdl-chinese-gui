...
![GUI](https://img.shields.io/badge/GUI-v1.4.0-16A085)
...
- 扫码后自动显示 Telegram 两步验证密码框；输入和输出均使用 Windows 隐藏终端，支持回车提交且不保存密码
- 可粘贴一整段文字，自动识别其中的 Telegram 消息链接
- 机器人 / 受保护聊天无需消息链接：加载近期媒体清单后，可按类型或关键词筛选并逐项勾选下载
- 无链接下载可查看和更改保存目录，开始前自动检查目录是否可写
- 登录或下载失败时显示具体处理阶段，并可一键复制经过隐私处理的完整错误信息
...
5. 在新窗口中选择机器人或聊天；机器人会自动排在列表最前。
6. 设置要加载的近期媒体数量（默认 30），检查保存位置后点击 **加载媒体**。
7. 在媒体清单中按视频、图片、音频或文件筛选，也可搜索文件名、消息说明或消息 ID；勾选后点击 **下载已选媒体**。
